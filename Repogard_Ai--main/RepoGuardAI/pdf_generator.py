...
def _styles() -> Dict[str, ParagraphStyle]:
    ...
    return {
        ...
        "code": ParagraphStyle("code", parent=sample["Code"], fontSize=7, leading=9, fontName="Courier", textColor=colors.HexColor("#1e293b")),
    }
...
def generate_pdf_report(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        story.append(Paragraph("11. Repository File Tree", styles["h2"]))
        story.append(Spacer(1, 0.3 * cm))
        story.append(Paragraph(
            "Directory and file structure as fetched from the default branch.",
            styles["small"],
        ))
        story.append(Spacer(1, 0.3 * cm))
        file_tree_text = _safe_text(repo_data.get("file_tree", "(Not available)"))
        tree_style = styles["code"]
        story.append(Paragraph("Legend: [DIR] folder, [FILE] file", styles["small"]))
        story.append(Spacer(1, 0.15 * cm))

        tree_lines = file_tree_text.splitlines() if file_tree_text else ["(Not available)"]
        if not tree_lines:
            tree_lines = ["(Not available)"]

        file_count = sum(1 for line in tree_lines if "[FILE]" in line)
        dir_count = sum(1 for line in tree_lines if "[DIR]" in line)
        stats_table = Table(
            [["Directories", str(dir_count)], ["Files", str(file_count)]],
            colWidths=[3.0 * cm, 2.2 * cm],
        )
        ...
        # Render in chunks to avoid oversized single flowable errors on large repositories.
        max_lines_per_block = 60
        for i in range(0, len(tree_lines), max_lines_per_block):
            chunk = "\n".join(tree_lines[i:i + max_lines_per_block])
            tree_table = Table(
                [[Preformatted(chunk, tree_style)]],
                colWidths=[15.8 * cm],
            )
            tree_table.setStyle(
                TableStyle(
            ...
            if i + max_lines_per_block < len(tree_lines):
                story.append(Spacer(1, 0.15 * cm))
        story.append(PageBreak())

        # 12) External API calls
        story.append(Paragraph("12. External API Calls Detected", styles["h2"]))
        story.append(Spacer(1, 0.3 * cm))
        story.append(Paragraph(
            "HTTP/HTTPS endpoints found by static scan of repository code files. "
            "Provider names are resolved from known hostname patterns.",
            styles["small"],
        ))
        story.append(Spacer(1, 0.3 * cm))
        api_calls = repo_data.get("api_calls", [])
        api_rows = [["#", "Provider", "Method", "Hostname", "File"]]
        if isinstance(api_calls, list) and api_calls:
            for idx, entry in enumerate(api_calls[:100], start=1):
                api_rows.append([
                    str(idx),
                    _safe_text(entry.get("provider")),
                    _safe_text(entry.get("method")),
                    _safe_text(entry.get("hostname")),
                    _safe_text(entry.get("file")),
                ])
        else:
            api_rows.append(["—", "No external API calls detected", "", "", ""])
        api_table = Table(
            api_rows,
            colWidths=[0.8 * cm, 3.2 * cm, 2.0 * cm, 4.6 * cm, 7.0 * cm],
            repeatRows=1,
        )
        api_table.setStyle(
            TableStyle(
        ...
        # 13) API key usage signals
        story.append(Paragraph("13. API Key Usage Signals", styles["h2"]))
        story.append(Spacer(1, 0.3 * cm))
        story.append(Paragraph(
            "Potential API key/token usage points detected in source code (env lookups, secret store lookups, "
            "and key-like variable assignments). This section highlights usage signals, not confirmed leaks.",
            styles["small"],
        ))
        story.append(Spacer(1, 0.3 * cm))

        key_usages = repo_data.get("api_key_usages", [])
        key_rows = [["#", "Key Name", "Usage Type", "File", "Line"]]
        if isinstance(key_usages, list) and key_usages:
            for idx, entry in enumerate(key_usages[:200], start=1):
                key_rows.append(
                    [
                        str(idx),
                        _safe_text(entry.get("key_name")),
                        _safe_text(entry.get("usage_type")),
                        _safe_text(entry.get("file")),
                        _safe_text(entry.get("line")),
                    ]
                )
        else:
            key_rows.append(["—", "No API key usage signals detected", "", "", ""])

        key_table = Table(
            key_rows,
            colWidths=[0.8 * cm, 3.2 * cm, 3.2 * cm, 7.8 * cm, 1.2 * cm],
            repeatRows=1,
        )
        key_table.setStyle(
            TableStyle(
