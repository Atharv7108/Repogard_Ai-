    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                # Longer exponential backoff for provider-side rate limits.
                sleep_seconds = min(10.0, 1.5 * (2 ** attempt))
...
def _collect_api_endpoints(repo_obj: Any, max_files: int = 120, max_endpoints: int = 120) -> List[Dict[str, Any]]:
...
def run_ai_analysis(repo_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    max_workers = int(os.getenv("AI_MAX_WORKERS", "1"))
